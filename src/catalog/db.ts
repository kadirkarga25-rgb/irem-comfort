...
export type Catalog = { id:string; title:string; year:string; season:string; description:string; cover?:string; pdf?:Blob; pdfUrl?:string; pageImages?:string[]; pageTexts?:string[]; pages:number; collections:Collection[]; contents:{title:string;page:number}[]; tags?:string[]; createdAt:number; published:boolean };
...
export async function saveCatalog(catalog:Catalog){const value:any={...catalog};if(value.pdf instanceof Blob){value.pdfUrl=await uploadData(catalog.id,value.pdf,`${catalog.id}.pdf`);}if(typeof value.cover==='string'&&value.cover.startsWith('data:')){const blob=await fetch(value.cover).then(r=>r.blob());value.cover=await uploadData(catalog.id,blob,`${catalog.id}-cover.${blob.type.includes('png')?'png':'jpg'}`);}delete value.pdf;delete value.pageImages;const r=await fetch(`${API}/catalogs/save`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${getCatalogAdminToken()}`},body:JSON.stringify({catalog:value})});const d=await r.json();if(!r.ok||!d.success)throw new Error(d.error||'Katalog kalıcı olarak kaydedilemedi.');}
